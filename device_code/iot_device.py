        ...
        try:
            ...
            if unsent_data:
                try:
                    ...
                    self.log_to_sd(f"Error decoding JSON data: {e}")
        ...
        while True:
            if not self.config_mode:
                try:
                    ...
                    if self.sd_error_count >= 10:
                        self.log_error_to_sd("SD card error count reached 10. Rebooting...")
                        machine.reset()
                except NetworkError:
                    self.reconnect_wifi()
                except SensorError:
